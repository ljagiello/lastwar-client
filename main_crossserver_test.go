...
// before this round, runCrossServerTest's save-back condition didn't compare AccessTok at all,
// so this exact case was silently dropped.
//
// Mutation-testing note: reverting crossServerSaveBackNeeded to its pre-fix form (dropping the
// `newAccessTok != origAccessTok` term, i.e. `return newHost != origHost || newPort != origPort
// || newZone != origZone`) makes the "access token alone changed" case below return false instead
// of true, failing this test -- proving the test actually exercises the fix rather than passing
// vacuously.
func TestCrossServerSaveBackNeeded(t *testing.T) {
	cases := []struct {
		name                                                     string
		newHost, newZone, newAccessTok, origHost, origZone, orig string
		newPort, origPort                                        int
		want                                                     bool
	}{
		{
			name:    "nothing changed",
			newHost: "1.2.3.4", newPort: 100, newZone: "APS1", newAccessTok: "tok",
			origHost: "1.2.3.4", origPort: 100, origZone: "APS1", orig: "tok",
			want: false,
		},
		{
			name:    "host changed (serverInfo redirect)",
			newHost: "5.6.7.8", newPort: 100, newZone: "APS1", newAccessTok: "tok",
			origHost: "1.2.3.4", origPort: 100, origZone: "APS1", orig: "tok",
			want: true,
		},
		{
			name:    "port changed",
			newHost: "1.2.3.4", newPort: 200, newZone: "APS1", newAccessTok: "tok",
			origHost: "1.2.3.4", origPort: 100, origZone: "APS1", orig: "tok",
			want: true,
		},
		{
			name:    "zone changed",
			newHost: "1.2.3.4", newPort: 100, newZone: "APS2", newAccessTok: "tok",
			origHost: "1.2.3.4", origPort: 100, origZone: "APS1", orig: "tok",
			want: true,
		},
		{
			...
			// this round's fix, runCrossServerTest's save-back condition never looked at the
			...
			newHost: "1.2.3.4", newPort: 100, newZone: "APS1", newAccessTok: "tok-fresh",
			origHost: "1.2.3.4", origPort: 100, origZone: "APS1", orig: "tok-stale",
			want: true,
		},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			got := crossServerSaveBackNeeded(c.newHost, c.newPort, c.newZone, c.newAccessTok, c.origHost, c.origPort, c.origZone, c.orig)
			if got != c.want {
