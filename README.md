...
| `sfsobject.go` | SFSObject/SFSArray binary codec (encode + decode), recursive pretty-printing. `String` itself is now safe by default -- it delegates to `StringRedacted`, so Go's automatic `fmt.Stringer` invocation (`%v`/`%s`, `Println`, slog's Any-kind formatting) can never leak a raw dump; the actual unredacted dump lives under the unexported `unsafeRawString`, which does not satisfy `fmt.Stringer` and is only meant to be called from within this file. `StringRedacted` masks known-sensitive fields (`loginKey`/`at`/`rt`/`accessToken`/`airKey`/`shumeiBoxId`/`pw`/`password`/`verifyCode`/`deviceId`/`chatToken`/`tk`/`ta`) -- including inside a sensitive key's own primitive-array or `SFSArray`-wrapped value -- so a decoded response can be logged for debugging without risking a credential leak. `maxDecodedNodes` charges each primitive array's actual element count (not a flat 1 per field) against the decode-size budget, except `sfsByteArray`, whose 1:1 wire-to-memory ratio needs no separate charge |
...
| `main.go` | CLI entrypoint; warns when `-decode-label` is set without `-decode-stream` (a no-op combination, mirroring this file's other ignored-flag warnings) |
...
| `sfsobject_redact_test.go` | `StringRedacted` masks every known-sensitive key (`loginKey`/`at`/`rt`/`accessToken`/`airKey`/`shumeiBoxId`/`pw`/`password`/`verifyCode`/`deviceId`/`chatToken`/`tk`/`ta`, including inside nested `SFSObject`/`SFSArray` values, a sensitive key's own raw `SFSArray`-wrapped value, and all 8 primitive-array wire types) while leaving ordinary gameplay fields untouched, matches `String`'s output exactly when no sensitive keys are present, `BuildLoginParams(IOSMode: true)` proven to never embed live DeviceID/AirKey/ShumeiBoxId secrets in its `ta` analytics blob, `fmt.Sprintf("%v", ...)` on a raw `*SFSObject` proven to never leak via Go's automatic `fmt.Stringer` invocation, and a legitimate large single `sfsByteArray` field proven to no longer spuriously fail against `maxDecodedNodes` |
