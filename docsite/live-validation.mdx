...
Everything documented elsewhere in this dossier up to this point was static analysis of a client the researcher owns — accurate, but unverified against a real server. This section documents what happened when the roadmap in [Go client implementation roadmap](/go-client-roadmap) was actually built (`goclient/`, a from-scratch Go reimplementation) and driven against the real production servers for a real account. It corrects a few static-analysis guesses, confirms most of them exactly, and documents one open problem static analysis alone could never have found.
...
| Reconnect into an *established* account's live game state (`CrossServerLogin`, triggered by picking a role after email bind) | **Confirmed** works cleanly once the `Login` request's identity fields (`packageName`, `packageSign`, `platform`, `pf`, `appVersion`, `versionCode`, `ta`, ...) match the access token's real issuing identity — see the reconnect wall below |
...
packageSign, platform="1" (hardcoded in this decompiled Android assembly -- not confirmed to hold "both platforms": the live iOS capture used for the reconnect fix above required platform="0", so this literal is almost certainly baked in per-platform build rather than shared), lat=0, device_string, pf,
...
<Check>
  **Resolved.** "Armed Truck" is not a building at all, and not `Scene/LWHummerScene` (a genuinely separate real-time driving/combat minigame, confirmed to have zero associated network commands anywhere in the ~3,178-command catalog — its rewards, if any, would require actually playing a timed 3D session, not a request/response call). It's also not `LW_BUILD_TRUCK_STATION` above, despite that being a real, suggestively-named lead. Confirmed via a live packet capture of the actual game client: "Armed Truck" is one of two independent tracks (the other is "Overlord") in a dedicated **idle/AFK reward panel**, collected through a single account-level command with no building `uuid` involved at all: `lw.pve.idle.reward`, with an `action` field — `0` peeks at the accumulated pool without claiming it, `1` claims it.
</Check>
...
Wired into the Go client as `CollectIdleReward` (`goclient/buildings.go`), called automatically at the start of every `-collect` run, independent of the building-uuid loop since this command needs no `uuid` at all.
...
Wired into the Go client as `HelpAllianceMembers` and `ClaimAllianceGifts` (`goclient/alliance.go`), both called automatically on every `-collect` run.
...
Wired into the Go client as `DonateRecommendedAllianceTech` (`goclient/alliance.go`), called automatically on every `-collect` run — one free donation attempt toward whatever the alliance currently has recommended, silently and correctly declining if the cooldown hasn't expired yet.
...
<Check>
  **Net effect.** Once both fixes landed, `init` arrived cleanly on the *first* login attempt in every post-fix login tested — no reconnect loop, no active-pull `login.init`, no extended timeout ever needed. The three-attempt-reconnect-plus-active-pull mechanism built earlier in this investigation to chase the missing-`init` symptom was solving the wrong problem; it's been reverted and disabled (max login attempts reduced back to 1), especially since repeated reconnect attempts against a stale or identity-mismatched token were themselves tripping `E005`/`E011` — meaning even "reconnect gets blocked after one attempt" was downstream of the same identity-mismatch root cause above, not a separate rate-limit or session-lock mechanism. No rate-limiting, no session-lock, no already-connected gate was ever found to exist anywhere in this path.
</Check>

**What's still open:** this is a proven-working recipe, not a fully general one yet. The `ta` analytics-JSON field's exact minimal required content is unconfirmed — only that the full real captured blob works; whether the server validates specific sub-fields inside it (a zone matching the top-level zone, a device id matching the top-level `deviceId`, ...) or just requires it to be non-trivially populated was never isolated, and a fresh captured token will need its own fresh `ta` capture, since it carries session-specific fields like a game-session id. This was also proven via the direct cross-server-test path with one specific captured access token — a from-scratch login using the iOS identity mode from the very start, including the GSL HTTP call, hasn't been tried yet, and neither has the mirror case: matching an *Android*-issued token's real identity fields for an Android-flavored reconnect.
