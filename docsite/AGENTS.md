...
- **`goclient`** — the researcher's from-scratch Go reimplementation of the protocol. Use the lowercase, no-space form (`goclient/identity.go`, `goclient` hardcoded...) when naming the actual codebase/package, and "Go client" (capitalized, two words) in ordinary prose ("a Go client needs to...").
