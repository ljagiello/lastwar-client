...
Built by reverse-engineering the decompiled APK; see `../docsite/live-validation.mdx` for the full
writeup of what's confirmed against live production servers (as opposed to static analysis alone).
...
- **Fully working, live-confirmed:** GSL crypto, SFS2X packet framing (including Zstandard
  ...
  `live-validation.mdx`), and a growing set of account-level automations: the "Armed Truck" idle
...
- The earlier "reconnect blocked" / "init push never arrives" problems were never protocol-level
  ...
  unhandled or unreachable on both login paths. See `live-validation.mdx` for the full root-cause
  writeups.
...
`../docsite/capturing-and-decoding-traffic.mdx` for the exact methodology. `gameUid`/`ip`/`port`/`zone`
...
`goclient/` or commit it anywhere).
...
# to obtain a fresh config -- see docsite/capturing-and-decoding-traffic.mdx for turning
...
- **Check the log's error rate periodically, not just whether the process is still running.** A
  ...
  actually catches this — see `docsite/live-validation.mdx`'s serverInfo-redirect section for a
...
| `tools/reassemble_stream.py` | Reassembles one TCP stream from a pcap into `-decode-stream`-ready files — see [Capturing and decoding traffic](../docsite/capturing-and-decoding-traffic.mdx) |
