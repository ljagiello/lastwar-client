...
// way conn_wait_test.go's session.NewPipeGameConnPair-based tests are. Instead these tests spin up a real
...
func TestDoCrossServerLoginHandshakeLogRedactsSessionToken(t *testing.T) {
	...
	addr := session.StartFakeGameServer(t, func(server *session.GameConn) {
		// First envelope: the Handshake request (controllerSystem/session.ActionHandshake, sent by
		...
		if err := server.SendEnvelope(session.ControllerSystem, session.ActionHandshake, hsResp); err != nil {
			return
		}
...
func withFailingDial(t *testing.T, n int, err error) {
	...
	dialGame = func(addr string, timeout time.Duration) (*session.GameConn, error) {
		conn, dialErr := session.DialGame(addr, timeout)
		if dialErr != nil {
			return nil, dialErr
		}
