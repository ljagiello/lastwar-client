...
func TestCloseConnBeforeExitClosesNonNilConnExactlyOnce(t *testing.T) {
	t.Run("non-nil conn closes exactly once", func(t *testing.T) {
		...
		conn := session.NewGameConnForTest(underlying)
...
func TestReadCodeFromPipeStatFailureClosesConnBeforeExit(t *testing.T) {
	if os.Getenv("LASTWAR_TEST_HELPER_PROCESS") == "1" {
		conn := session.NewGameConnForTest(&markerCloseConn{})
...
func TestDedupeBuildingsCapsAggregateCount(t *testing.T) {
	t.Run("exactly cap distinct uuids: all kept, no truncation warning", func(t *testing.T) {
		...
		if len(got) != game.MaxAggregateBuildingsPerFetch {
			t.Fatalf("got %d buildings, want exactly %d (the cap, no truncation at this boundary)", len(got), game.MaxAggregateBuildingsPerFetch)
		}
	...
	t.Run("cap+1 distinct uuids: truncated to cap, warning fires", func(t *testing.T) {
		...
		if len(got) != game.MaxAggregateBuildingsPerFetch {
			t.Fatalf("got %d buildings, want exactly %d (cap+1 input must still truncate to the cap)", len(got), game.MaxAggregateBuildingsPerFetch)
		}
...
func TestDedupeVisitorsCapsAggregateCount(t *testing.T) {
	t.Run("exactly cap distinct uids: all kept, no truncation warning", func(t *testing.T) {
		...
		if len(got) != game.MaxVisitorsUpperBound {
			t.Fatalf("got %d visitors, want exactly %d (the cap, no truncation at this boundary)", len(got), game.MaxVisitorsUpperBound)
		}
		if strings.Contains(buf.String(), "longer than aggregate cap") {
			t.Errorf("unexpected truncation warning at exactly-cap boundary:\n%s", buf.String())
		}
	...
	t.Run("cap+1 distinct uids: truncated to cap, warning fires", func(t *testing.T) {
		...
		if len(got) != game.MaxVisitorsUpperBound {
			t.Fatalf("got %d visitors, want exactly %d (cap+1 input must still truncate to the cap)", len(got), game.MaxVisitorsUpperBound)
		}
