...
// SFS2X server helpers (session.NewFakeGameListener/session.ServeFakeGameServer/session.StartFakeGameServer/
// putRedirectServerInfo/testutil.SplitHostPortInt) for the SFS2X side, and add a small combined fake HTTP
...
// fakeInitPushServerWithDuplicateUUIDs mirrors session.FakeInitPushServer above, but the `init` push it
...
func fakeInitPushServerWithDuplicateUUIDs() func(*session.GameConn) {
	return func(server *session.GameConn) {
		...
		b1 := sfs.NewSFSObject()
		b1.PutLong("uuid", 111)
		b1.PutInt("bId", game.BuildingFarmland)
		b1Dup := sfs.NewSFSObject()
		b1Dup.PutLong("uuid", 111)
		b1Dup.PutInt("bId", game.BuildingFarmland)
		b2 := sfs.NewSFSObject()
		b2.PutLong("uuid", 222)
		b2.PutInt("bId", game.BuildingIronMine)
...
func TestLoginDedupesInitPushBuildingsAndVisitors(t *testing.T) {
	...
	addr := session.StartFakeGameServer(t, fakeInitPushServerWithDuplicateUUIDs())
	host, port := testutil.SplitHostPortInt(t, addr)
...
// fakeInitPushServerWithWrongTypedBuildingUUID mirrors session.FakeInitPushServer above, but the `init`
...
func fakeInitPushServerWithWrongTypedBuildingUUID() func(*session.GameConn) {
	return func(server *session.GameConn) {
		if _, err := server.ReadEnvelope(); err != nil {
			return
		}
...
func TestLoginRejectsWrongTypedBuildingUUID(t *testing.T) {
	...
	addr := session.StartFakeGameServer(t, fakeInitPushServerWithWrongTypedBuildingUUID())
	host, port := testutil.SplitHostPortInt(t, addr)
	...
	if result.Buildings[0].Uuid() != 0 || result.Buildings[0].BId() != game.BuildingIronMine {
		t.Errorf("got building uuid=%d bId=%d, want the genuine uuid=0 bId=%d (BuildingIronMine) entry -- got the wrong-typed entry's bId instead if this reads BuildingFarmland", result.Buildings[0].Uuid(), result.Buildings[0].BId(), game.BuildingIronMine)
	}
}
...
// this file drives its fake server through session.FakeInitPushServer or an equivalent send-the-init-push
...
func TestLoginTooManyRedirects(t *testing.T) {
	...
	for i := range lns {
		ln, addr := session.NewFakeGameListener(t)
		lns[i] = ln
		addrs[i] = addr
	}
	for i, ln := range lns {
		session.ServeFakeGameServer(ln, func(server *session.GameConn) {
			...
			_ = server.SendEnvelope(session.ControllerSystem, session.ActionLogin, putRedirectServerInfo(next, zone))
		})
	}

	host, port := testutil.SplitHostPortInt(t, addrs[0])
	gsl := testutil.NewFakeGSLServer(t, gsl.LoginServerListRespon{
		Code:       "0",
		ServerList: []gsl.LoginServerInfo{{IP: gsl.FlexString(host), Port: testutil.FlexPort(port), Zone: "APS0", GameUid: "uid-1"}},
		At:         &gsl.LoginToken{Token: "tok-1"},
	})
	testutil.UseFakeGSLServer(t, gsl)
...
func TestLoginExactlyMaxRedirectHopsSucceeds(t *testing.T) {
	...
	for i, ln := range lns {
		session.ServeFakeGameServer(ln, func(server *session.GameConn) {
			...
			if i+1 < numServers {
				zone := fmt.Sprintf("APS%d", i+1)
				_ = server.SendEnvelope(session.ControllerSystem, session.ActionLogin, putRedirectServerInfo(addrs[i+1], zone))
				return
			}
...
func buildLoggableServerList(host string, port int) []gsl.LoginServerInfo {
	...
	list[0] = gsl.LoginServerInfo{IP: gsl.FlexString(host), Port: testutil.FlexPort(port), Zone: "APS0", GameUid: "uid-1"}
	for i := 1; i < len(list); i++ {
		list[i] = gsl.LoginServerInfo{IP: "10.0.0.1", Port: testutil.FlexPort(9999), Zone: "APS0", GameUid: gsl.FlexString(fmt.Sprintf("uid-unreachable-%d", i))}
	}
	return list
}
...
func TestLoginEmailVerificationPath(t *testing.T) {
	...
	addr := session.StartFakeGameServer(t, func(server *session.GameConn) {
		...
		// followed by the `init` push (same session.FakeInitPushServer pattern used above) so step 5
		...
		sendCodeMsg, err := session.ReadNextExtension(server)
		if err != nil {
			return
		}
		...
		finishMsg, err := session.ReadNextExtension(server)
		if err != nil {
			return
		}
	...
	gsl := testutil.NewFakeGSLServer(t, gsl.LoginServerListRespon{
		Code:       "0",
		ServerList: []gsl.LoginServerInfo{{IP: gsl.FlexString(host), Port: testutil.FlexPort(port), Zone: "APS1", GameUid: ""}},
		At:         &gsl.LoginToken{Token: "tok-1"},
	})
	testutil.UseFakeGSLServer(t, gsl)
...
// unsafe here per session.StartFakeGameServer's own doc comment, since the handler goroutine can outlive
// the test.
...
// second fake server (session.FakeInitPushServer) completes the redialed login. Proves (1) the redirect to
...
func TestLoginRejectsOversizedInitialZoneAccessTokAndGameUid(t *testing.T) {
	...
	addr := session.StartFakeGameServer(t, session.FakeInitPushServer(gotZn))
	host, port := testutil.SplitHostPortInt(t, addr)
	...
	gsl := testutil.NewFakeGSLServer(t, gsl.LoginServerListRespon{
		Code:       "0",
		ServerList: []gsl.LoginServerInfo{{IP: gsl.FlexString(host), Port: testutil.FlexPort(port), Zone: oversizedZone, GameUid: oversizedGameUid}},
