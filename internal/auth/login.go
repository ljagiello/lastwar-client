...
func Login(opts LoginOptions) (*LoginResult, error) {
	...
	var conn *session.GameConn
	var buildings []game.Building
	var visitors []game.Visitor
	var initErr error
	gotInit := false
	redirectHops := 0
	...
	for attempt := 1; attempt <= maxLoginAttempts; attempt++ {
		...
		if ec, ok := env.Content.Get("ec"); ok {
			_ = conn.Close()
			return nil, fmt.Errorf("LOGIN FAILED: ec=%v full=%s: %w", ec.Val, env.Content.StringRedacted(), session.ErrAuthRejected)
		}
		// session.WarnIfWrongTypedField below adds a diagnostic for the "field present but wrong-typed"
		...
		session.WarnIfWrongTypedField(env.Content, "un", "base-zone login response", session.SFSFieldKindString)
	...
	msg, err := session.WaitForCmd(conn, 15*time.Second, "account.login.send.verify.code", "push.account.send.verify.code")
	if err != nil {
		_ = conn.Close()
		return nil, err
	}
	if ec, ok := msg.Params.Get("errorCode"); ok {
		_ = conn.Close()
		return nil, fmt.Errorf("SEND-CODE FAILED: errorCode=%v full=%s: %w", ec.Val, msg.Params.StringRedacted(), session.ErrAuthRejected)
	}
	...
	ackMsg, err := session.WaitForCmd(conn, 15*time.Second, "account.login.new")
	if err != nil {
		_ = conn.Close()
		return nil, err
	}
	if ec, ok := ackMsg.Params.Get("errorCode"); ok {
		_ = conn.Close()
		return nil, fmt.Errorf("LOGIN-WITH-CODE FAILED: errorCode=%v full=%s: %w", ec.Val, ackMsg.Params.StringRedacted(), session.ErrAuthRejected)
	}
	...
	msg2, err := session.WaitForCmd(conn, 15*time.Second, "push.account.login.new")
	if err != nil {
		_ = conn.Close()
		return nil, err
	}
	if ec, ok := msg2.Params.Get("errorCode"); ok {
		...
		return nil, fmt.Errorf("LOGIN-WITH-CODE FAILED (push): errorCode=%v gameUid=%s loginKey=%s: %w",
			ec.Val, msg2.Params.GetString("gameUid"), sfs.Redact(msg2.Params.GetString("loginKey")), session.ErrAuthRejected)
	}
	...
	session.WarnIfWrongTypedField(msg2.Params, "loginKey", "push.account.login.new", session.SFSFieldKindString)
	...
	session.WarnIfWrongTypedField(msg2.Params, "gameUid", "push.account.login.new", session.SFSFieldKindString)
	if gu := msg2.Params.GetString("gameUid"); gu != "" {
		if err := ident.SaveGameUid(gu); err != nil {
			slog.Warn("failed to persist gameUid", "error", err)
		}
	}
	session.WarnIfWrongTypedField(msg2.Params, "gameUserName", "push.account.login.new", session.SFSFieldKindString)
...
func waitForInitPush(conn *session.GameConn, timeout time.Duration) ([]game.Building, []game.Visitor, bool, error) {
	...
	for {
		...
		if ok && msg.Cmd == "init" {
			return dedupeBuildings(game.ParseInitBuildings(msg.Params)), dedupeVisitors(game.ParseInitVisitors(msg.Params)), true, nil
		}
...
// init push.
func dedupeBuildings(bs []game.Building) []game.Building {
	var out []game.Building
	seen := make(map[int64]bool, len(bs))
	truncated := false
	for _, b := range bs {
		if len(out) >= game.MaxAggregateBuildingsPerFetch {
			truncated = true
			break
		}
		uuid := b.Uuid()
	...
	if truncated {
		slog.Warn("init push building_new longer than aggregate cap after dedup; truncating",
			"rawCount", len(bs), "cap", game.MaxAggregateBuildingsPerFetch)
	}
	return out
}
...
func dedupeVisitors(vs []game.Visitor) []game.Visitor {
	var out []game.Visitor
	seen := make(map[int64]bool, len(vs))
	truncated := false
	for _, v := range vs {
		if len(out) >= game.MaxVisitorsUpperBound {
			truncated = true
			break
		}
		uid := v.Uid()
	...
	if truncated {
		slog.Warn("init push visitor.list longer than aggregate cap after dedup; truncating",
			"rawCount", len(vs), "cap", game.MaxVisitorsUpperBound)
	}
	return out
}

func readCodeFromStdin(conn *session.GameConn) string {
...
func readCodeFromPipe(path string, conn *session.GameConn) string {
...
func closeConnBeforeExit(conn *session.GameConn) {
	if conn != nil {
		_ = conn.Close()
	}
}
...
func readCodeFrom(r io.Reader, closer io.Closer, conn *session.GameConn) string {
