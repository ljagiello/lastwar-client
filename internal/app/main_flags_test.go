...
// doc comment for why that's correct, shared behavior) -- but before this round's fix, Run() never
...
// Run() without also adding it to crossServerFlagNames (so it silently falls through the
// -cs-*-ignored warning with no diagnostic), or a stale name left in crossServerFlagNames after a
// flag is renamed or removed from Run().
//
...
// ground truth for what's actually registered on the FlagSet in Run().
func TestCrossServerFlagNamesMatchesDeclarations(t *testing.T) {
	...
	for name := range declared {
		...
		if !crossServerFlagNames[name] {
			t.Errorf("flag %q is declared on the FlagSet in Run() but missing from crossServerFlagNames -- add it there (or to this test's gating set, if it's meant to gate the cross-server path like -cs-ip/-cs-rt)", name)
		}
	}
	for name := range crossServerFlagNames {
		if !declared[name] {
			t.Errorf("crossServerFlagNames recognizes %q but no such flag is declared on the FlagSet in Run() -- remove it, or check whether the flag was renamed", name)
		}
	}
}
...
// on the FlagSet in Run(), rather than a second hand-maintained list that would just reproduce
// the same drift risk this test exists to catch -- and asserts stringFlagSwallowGuardNames' keys
// exactly match. This fails if a future fs.String flag is added to Run() without also adding it
...
func TestStringFlagSwallowGuardNamesMatchesDeclarations(t *testing.T) {
	...
	for name := range declared {
		if !stringFlagSwallowGuardNames[name] {
			t.Errorf("flag %q is declared fs.String on the FlagSet in Run() but missing from stringFlagSwallowGuardNames -- add it there (see detectSwallowedFlagValue's doc comment for what this guard protects against)", name)
		}
	}
	for name := range stringFlagSwallowGuardNames {
		if !declared[name] {
			t.Errorf("stringFlagSwallowGuardNames recognizes %q but no such fs.String flag is declared on the FlagSet in Run() -- remove it, or check whether the flag was renamed", name)
		}
	}
}

// TestShouldAbortBeforeInteractive is the fast, deterministic unit test of the decision extracted
// from both -collect call sites (Run() and runCrossServerTest) as this round's Fix 1:
