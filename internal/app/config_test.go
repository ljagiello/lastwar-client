...
// Run()'s -cs-ip/-cs-gameuid config-merge used to silently replace an explicitly-passed-but-empty
...
// own extraction, so the decision itself is directly testable independent of Run()'s wiring.
