...
// the MINOR finding that Run() and runCrossServerTest() each register `defer conn.Close()` right
...
// Run()'s very first statement (installing a placeholder JSON handler before any flag is even
// declared) exists specifically to guarantee. Captures slog's output through a JSON handler (the
// same shape Run() itself installs) and asserts the diagnostic reaches THAT sink, structured, with
...
// documented by the comment above fs.Parse near the top of Run() (main.go): -h/-help exits 0,
...
// handling further down in Run()). This contract is operationally load-bearing -- README's cron
// examples check $? to decide whether to alert -- but until this test, it had zero coverage:
// main_flags_test.go only exercises pure helper functions, never fs.Parse/os.Exit inside Run()
// itself.
//
// Run() reads os.Args and calls os.Exit directly, so (like TestRunCrossServerTestExitsWhenIPEmpty
...
// options struct here -- the flag-parsing logic lives inline in Run() itself -- so instead of
// calling a helper, this reuses the same re-exec-the-test-binary-as-a-subprocess idiom but has the
// child overwrite os.Args to the exact argv Run() would see for a real invocation before calling
// Run() itself. Every case below exits either inside fs.Parse's own error branch, in the
...
func TestMainFlagParseExitCodes(t *testing.T) {
	if os.Getenv("LASTWAR_TEST_HELPER_PROCESS") == "1" {
		...
		os.Args = argv
		Run()
		// Run() always os.Exits before returning for every argv this test drives it with; only
	...
	cases := []struct {
		...
		// stderr from INSIDE fs.Parse, before Run() ever saw the returned error, so no
		// error-handling branch in Run() could have intercepted it even after round 32's fix.
		...
		{"malformed flag value exits 1", []string{"-cs-port=not-a-number"}, 1, "", true},
		{
			...
			// check added in Run() (right after fs.Parse succeeds), this exact invocation used to
			...
			// "-collect"). It's safe to drive through the real Run() here, with no fake servers or
		...
		},
		{
			...
			// Run() here, with no fake servers or HOME override, since the new check exits
...
// call site, which had zero test coverage: unlike every other Run() exit path exercised by
// TestMainFlagParseExitCodes above, -version deliberately returns from Run() normally (see its
...
// secret value directly in this log line, in cleartext. Drives a real Run() invocation (like
...
// Run()'s config-merge block -- -cs-zone/-cs-deviceid/-cs-shumei/-cs-at were left on the old bare
...
//
// This doubles as the regression test for the sibling MAJOR finding that Run()'s actual
...
// crossServerTestOpts struct directly, bypassing Run()'s own config-loading and merge code
// entirely.
//
// Drives a real Run() invocation (like TestMainFlagParseExitCodes) with a temp session config
...
func TestMainConfigMergeExplicitlyEmptyFlagsWarnAndSkipConfigFallback(t *testing.T) {
	if os.Getenv("LASTWAR_TEST_HELPER_PROCESS_CONFIG_MERGE") == "1" {
		...
		Run()
		// Only reached if Run() fails to exit -- the outer assertions below will then see a clean
...
// passing unchanged). -cs-ip gates whether Run() takes the cross-server branch at all
...
//
// TestMainFlagParseExitCodes above proves this is actually wired into Run()'s own fs.Visit call
...
// o.portExplicit (crossServerTestOpts' own visitedFlags-derived field, populated in Run() via the
...
// call site (main_crossserver_test.go), but Run()'s OWN -collect+-interactive call site --
...
// hitting one injected business-logic (non-net.Error) failure, and confirms Run() does NOT
...
func TestMainCollectInteractiveCallSiteReachesRunInteractiveDespiteBusinessLogicError(t *testing.T) {
	if os.Getenv("LASTWAR_TEST_HELPER_PROCESS") == "1" {
		...
		Run()
		// Only reached if Run() fails to exit -- the outer assertions below will then see a
	...
	if !strings.Contains(log, "interactive mode: reading commands") {
		t.Errorf("subprocess stderr = %s\nwant it to contain RunInteractive's startup log -- proof Run()'s own -collect+-interactive call site actually reached RunInteractive instead of aborting on CollectAll's business-logic error", log)
	}
...
// genuine silence timeout), which is what actually reaches Run()'s zero-buildings FetchBuildings
...
// regression test for Fix 1's second call site: FetchBuildings' fallback call in Run() itself
...
func TestMainFetchBuildingsFallbackFailureWithInteractiveReachesRunInteractive(t *testing.T) {
	...
	if !strings.Contains(log, "interactive mode: reading commands") {
		t.Errorf("subprocess stderr = %s\nwant it to contain RunInteractive's startup log -- proof Run()'s FetchBuildings fallback call site actually reached RunInteractive instead of unconditionally aborting on the fetch failure", log)
	}
	if !strings.Contains(log, "stat control pipe failed") {
		t.Errorf("subprocess stderr = %s\nwant RunInteractive's own bogus-control-pipe failure -- confirms the exit code 1 came from there, not from some earlier, different failure", log)
	}
...
// would abort before CollectAll ever ran): Run()'s zero-buildings FetchBuildings fallback call
...
func TestMainZeroBuildingsFallbackPreservesNonEmptyVisitors(t *testing.T) {
	if os.Getenv("LASTWAR_TEST_HELPER_PROCESS") == "1" {
		...
		os.Args = []string{"lastwar-client", "-collect", "-interactive", "/nonexistent/lastwar-test-control-pipe"}
		Run()
		// Run() always os.Exits before returning on this path (RunInteractive's own os.Stat
