...
func TestAESECBRoundTrip(t *testing.T) {
	key := crypto.MD5HexKey("test-salt-value-1234")
	if len(key) != 32 {
		t.Fatalf("expected 32-byte key, got %d", len(key))
	}
	plain := []byte(`uuid=abc&airKey=lwDid_xyz&loginFlag=1`)
	ct, err := crypto.AESECBEncryptPKCS7(plain, key)
	if err != nil {
		t.Fatalf("encrypt: %v", err)
	}
	pt, err := crypto.AESECBDecryptPKCS7(ct, key)
	if err != nil {
	...
	ct2, _ := crypto.AESECBEncryptPKCS7(plain, key)
...
// for the AES-256-ECB use here -- see crypto.AESECBDecryptPKCS7's bs := block.BlockSize()).
// crypto.PKCS7Unpad's old bound only checked padLen <= len(data), which a
...
func TestPkcs7UnpadRejectsPadLenAboveBlockSize(t *testing.T) {
	...
	// it. Encrypt it directly (bypassing crypto.PKCS7Pad) so that decrypting via
	// crypto.AESECBDecryptPKCS7 reproduces this exact "corrupted" plaintext, with
	// its last byte intact, for crypto.PKCS7Unpad to evaluate.
	...
	if _, err := crypto.AESECBDecryptPKCS7(ct, key); err == nil {
...
// finding that crypto.PKCS7Unpad's `padLen > blockSize` guard (crypto.go) had no exact-boundary test:
...
// (must be ACCEPTED -- it's the pad length crypto.PKCS7Pad itself produces whenever the plaintext is
...
func TestPkcs7UnpadPadLenEqualsBlockSizeBoundary(t *testing.T) {
	...
	t.Run("padLen exactly blockSize: accepted, a full block of padding is stripped", func(t *testing.T) {
		// A plaintext whose length is already a multiple of bs forces crypto.PKCS7Pad to append an
		...
		// crypto.AESECBEncryptPKCS7/crypto.PKCS7Pad path rather than being hand-crafted.
...
// crypto.PKCS7Unpad's `padLen <= 0` half of its guard (`if padLen <= 0 || padLen > len(data)`) had zero
...
// PKCS7 pad length is always >= 1). Encrypts a crafted plaintext directly (bypassing crypto.PKCS7Pad, the
// same technique TestPkcs7UnpadRejectsPadLenAboveBlockSize/TestPkcs7UnpadRejectsMismatchedPaddingBytes
// use) whose last byte is 0x00, and proves crypto.AESECBDecryptPKCS7 rejects it instead of silently
...
// TestPkcs7UnpadRejectsMismatchedPaddingBytes covers crypto.PKCS7Unpad's byte-by-byte padding-match
...
// produce after AES decryption. Encrypting this crafted plaintext directly (bypassing crypto.PKCS7Pad, as
// the sibling test above does) and decrypting via crypto.AESECBDecryptPKCS7 reproduces it verbatim for
// crypto.PKCS7Unpad to reject.
...
// TestPkcs7UnpadRejectsEmptyInput is the round-50 regression test for crypto.PKCS7Unpad's leading
// `len(data) == 0` guard (crypto.go), which had zero direct test coverage: every existing
// crypto.PKCS7Unpad test above only reaches it indirectly through crypto.AESECBDecryptPKCS7, which itself
// already rejects a zero-length ciphertext one guard earlier (see
// TestAESECBDecryptPKCS7RejectsBadCiphertextLength below), so crypto.PKCS7Unpad's own empty-input branch
// was never actually exercised by any of them. Calls crypto.PKCS7Unpad directly with a nil slice, the one
// shape that can only ever reach this specific guard.
func TestPkcs7UnpadRejectsEmptyInput(t *testing.T) {
	if _, err := crypto.PKCS7Unpad(nil, 16); err == nil {
		t.Fatal("expected an error for empty input, got nil")
	}
}

// TestAESECBDecryptPKCS7RejectsBadCiphertextLength covers crypto.AESECBDecryptPKCS7's ciphertext-length
...
func TestAESECBDecryptPKCS7RejectsBadCiphertextLength(t *testing.T) {
	...
	t.Run("empty ciphertext", func(t *testing.T) {
		if _, err := crypto.AESECBDecryptPKCS7(nil, key); err == nil {
