...
// all use (session.NewPipeGameConnPair/session.ReadAndReply, conn_wait_test.go). Pure, network-free logic
...
func TestFetchBuildingsConsecutiveDecodeFailuresBoundary(t *testing.T) {
	...
	t.Run("exactly cap consecutive corrupt frames: init push still parsed", func(t *testing.T) {
		buildings, _, err := sendCorruptThenInit(t, session.MaxConsecutiveDecodeFailures)
		if err != nil {
	...
	t.Run("cap+1 consecutive corrupt frames: gives up before reaching the init push", func(t *testing.T) {
		buildings, logged, err := sendCorruptThenInit(t, session.MaxConsecutiveDecodeFailures+1)
		if err == nil {
...
func TestFetchBuildingsNonMatchingEnvelopeCapBoundary(t *testing.T) {
	...
	t.Run("exactly cap non-matching pushes: init push still parsed", func(t *testing.T) {
		buildings, err := sendNoiseThenInit(t, session.MaxNonMatchingEnvelopesPerWait)
		if err != nil {
			t.Fatalf("FetchBuildings() error = %v, want nil (exactly the cap must still be tolerated)", err)
		}
		if len(buildings) != 1 || buildings[0].Uuid() != 111 {
	...
	t.Run("cap+1 non-matching pushes: gives up benignly", func(t *testing.T) {
		buildings, err := sendNoiseThenInit(t, session.MaxNonMatchingEnvelopesPerWait+1)
		if err == nil {
...
func TestFetchBuildingsInitPushParsesBuildingsAndVisitors(t *testing.T) {
	...
	go func() {
		...
		_ = server.RawConn().Close() // see doc comment above: ends the test fast instead of waiting out the post-init 3s window
	}()
...
func TestFetchBuildingsDedupesBuildingUUIDAcrossSources(t *testing.T) {
	...
	go func() {
		...
		_ = server.RawConn().Close() // see TestFetchBuildingsInitPushParsesBuildingsAndVisitors' doc comment: ends the test fast instead of waiting out the post-push 3s window
	}()
...
func TestFetchBuildingsWaitsForAuthoritativeInitDespiteEarlyPushInitBuild(t *testing.T) {
	...
	go func() {
		...
		_ = server.RawConn().Close() // see TestFetchBuildingsInitPushParsesBuildingsAndVisitors' doc comment: ends the test fast instead of waiting out the post-init 3s window
	}()
...
// keep this call (and therefore the whole synchronous Run() flow) hanging indefinitely, a
...
func TestCollectAllAbortsRemainingActionsOnNetError(t *testing.T) {
	...
	client := session.NewGameConnForTest(fake)
...
func TestContainsNonTimeoutNetError(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := session.ContainsNonTimeoutNetError(tt.err); got != tt.want {
