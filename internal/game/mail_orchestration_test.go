...
func TestListMailPaginates(t *testing.T) {
	...
	go func() {
		...
		readReq := func() (*session.ExtensionMessage, bool) {
...
func mailBatchServer(t *testing.T, server *session.GameConn, mails []*sfs.SFSObject, wantReadBatches, wantRewardBatches int) (readBatches, rewardBatches [][]string, rewardType int32) {
	t.Helper()
...
func TestClaimAllMailProcessesPartialMailOnListPageFailure(t *testing.T) {
	...
	readReq := func(wantCmd string) *session.ExtensionMessage {
		t.Helper()
		env, err := server.ReadEnvelope()
		if err != nil {
...
// net.Pipe round trip: wrap it in a bare NewGameConnForTest(rec) (SendExtension/SendEnvelope only ever
...
func encodeResponse(t *testing.T, cmd string, resp *sfs.SFSObject) []byte {
	t.Helper()
	rec := &recordingConn{}
	recorder := session.NewGameConnForTest(rec)
