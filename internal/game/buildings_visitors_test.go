...
func TestPrintBuildings(t *testing.T) {
	...
	out := testutil.CaptureStdout(t, func() { PrintBuildings(buildings) })
...
// session.NewPipeGameConnPair/SendExtension fake-server pattern as buildings_orchestration_test.go's other
...
// pattern, buildings_orchestration_test.go/conn_wait_test.go's session.NewPipeGameConnPair/SendExtension)
...
// through a live push.add.building push via FetchBuildings, the same session.NewPipeGameConnPair/
...
func TestSfsFieldKindAccepts(t *testing.T) {
	...
	t.Run("sfsFieldKindString", func(t *testing.T) {
		if !session.SFSFieldKindAccepts(session.SFSFieldKindString, "hello") {
