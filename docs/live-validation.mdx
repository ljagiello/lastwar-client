...
| Reconnect into an *established* account's live game state (`CrossServerLogin`, triggered by picking a role after email bind) | **Confirmed** at the time of capture, works cleanly once the `Login` request's identity fields (`packageName`, `packageSign`, `platform`, `pf`, `appVersion`, `versionCode`, `ta`, ...) match the access token's real issuing identity — that capture used `ta`'s real captured sub-fields; the shipped code has since replaced those sub-fields with placeholders for security reasons, and whether reconnect still succeeds with placeholders is unconfirmed — see the reconnect wall below and "What's still open" further down |
...
**What's still open:** this was a proven-working recipe at the time it was captured, not a fully general one, and — against the code as currently shipped — not a currently-confirmed one either. The `ta` analytics-JSON field's exact minimal required content was never isolated: only that the *full real captured* blob worked, at the time of that capture; whether the server validates specific sub-fields inside it (a zone matching the top-level zone, a device id matching the top-level `deviceId`, ...) or just requires it to be non-trivially populated was never determined. Since that capture, a subsequent security audit deliberately replaced three of those sub-fields — `LwDeviceID`, `LwShumeiID`, `LwAirKey` in `identity.go`'s `BuildLoginParams` (`IOSMode` branch) — with empty-string placeholders, because their real values leaked the same live `DeviceID`/`ShumeiBoxId`/`AirKey` secrets in cleartext inside an opaque JSON string that `StringRedacted()`'s key-based masking couldn't see into. That fix's own comment states the placeholders stand in "until/unless a live capture confirms the server actually validates their content" — meaning whether reconnect still succeeds with the placeholder-based blob, in the code as it ships today, is genuinely unconfirmed. It's an open question, not a proven-working recipe, until a fresh live capture settles it one way or the other. That fresh capture would also need its own fresh `ta` regardless, since the blob carries session-specific fields like a game-session id. This was also only proven via the direct cross-server-test path with one specific captured access token — a from-scratch login using the iOS identity mode from the very start, including the GSL HTTP call, hasn't been tried yet, and neither has the mirror case: matching an *Android*-issued token's real identity fields for an Android-flavored reconnect.
